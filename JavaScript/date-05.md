...
2월의 말일자인 2월 28일(또는 2월 29일, 윤달인 경우)을 구하기 위해  
1월 31일 + 1개월을 계산하면  
...
월의 마지막 일자를 계산하는 방법을 응용해서 말일자를 체크하는 함수를 작성하고,  
...
말일 자 처리를 위해서 addMonth(date, month)라는 함수를 만든다.  
...
4. 파라미터로 받은 기준 일자의 날짜와 2에서 구한 날짜의 date 값(즉, 2자리 일자)을 비교하여 작은 값을 리턴 값의 date 값으로 할당해 준다.  
...
보통은 월을 계산할 때 발생하는 말일 자 문제(윤달, 윤년 포함)를 해결하는 코드를 직접 로직을 구현해서 사용해도 되지만,  
