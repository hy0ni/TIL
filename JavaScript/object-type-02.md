...
파라미터로 전달된 property(속성)가 객체에 존재하면 true를 리턴하고,<br>
그렇지 않으면 false를 리턴한다.
...
in 연산자도 hasOwnProperty() 함수와 비슷하게 객체에 프로퍼티가 있으면 true를 리턴하고,<br>
그렇지 않으면 false를 리턴한다.
...
`toString` 메서드는 모든 객체가 공통적으로 가지는 속성이다.<br>
`hasOwnProperty()` 함수는, 객체로부터 상속받은 속성을 체크하면 false를 리턴한다.<br>
...
'person.age'와 같이, 객체에 존재하지 않는 속성(property)에 접근하면 undefined가 리턴된다.<br>
...
person 객체에 age 속성이 존재하지만, 그 값이 undefined로 정의되어 있을 경우<br>
person.age를 undefined 값과 비교하면 true를 리턴한다.

person 객체에 age 속성이 존재하지만, 속성값이 undefined이기 때문에,<br>
