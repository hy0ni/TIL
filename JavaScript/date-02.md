...
- `월은 0에서 11 사이의 숫자(0~11)가 입력` 되어야 한다.<br>
  0은 1월, 11은 12월이다.
...
그런데, 자세히 보면 `1970년 1월 1일 0시 0분 1초`가 아닌,<br>
`1970년 1월 1일 9시 0분 1초`인 이유는<br>
우리나라는 UTC 시간과 9시간의 시차가 나기 때문에 1970년 1월 1일 9시 0분 1초라고 나온다.

즉, milliseconds를 파라미터로 전달하면<br>
1970년 1월 1일 0시 0분 0초 0밀리초를 기준으로,<br>
입력된 millisecond 만큼 지난 후의 날짜를 가진 Date 객체를 생성하게 된다.<br>
그리고, 생성된 Date 객체를 출력하면,<br>
