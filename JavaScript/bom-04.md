...
`const url = new URL("URL문자열(document.location)")`<br>
...
`new URLSearchParams("쿼리스트링")`<br>
URLSearchParams에 쿼리스트링을 전달하여,<br>
URLSearchParams 객체를 직접 생성할 수 있다.

`params.has('파라미터명')`<br>
params.has() 함수를 사용하면,<br>
...
파라미터 추가: `params.append("파라미터명", "값")`<br>
파라미터 변경: `params.set("파라미터명", "값")`<br>
파라미터 삭제: `params.delete("파라미터명")`<br>
