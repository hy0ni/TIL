...
`element.className += ' 이름'`  
기존의 클래스에 값을 추가하려면, 기존의 className에 추가하려는 값을 문자열로 연결해 주면 된다.  
...
element.classList.add() 메서드를 사용해서 class를 추가할 수 있다.  
element.classList.add() 메서드는 문자열 앞에 스페이스를 입력하지 않아도 된다.  
