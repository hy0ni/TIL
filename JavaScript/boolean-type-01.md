...
| ===    |    일치 연산자    | 왼쪽과 오른쪽의 값이 완전히 동일한지 테스트. (값과 타입이 모두 같은 경우 true를 반환한다.)  | 5 === 2 + 4 |
| !==    |   불일치 연산자   | 왼쪽과 오른쪽 값이 서로 동일하지 않은지 테스트. (값 또는 타입이 다를 경우 true를 반환한다.) | 5 !== 2 + 3 |
...
==와 != 동등성과 불일치성을 테스트하는 데 사용한다.  
==와 != 값이 동일한지는 테스트하지만, 값의 데이터 유형이 동일한지의 여부는 테스트하지 않는다.

===와 !== 값과 값의 데이터 유형의 동일성을 모두 테스트한다.  
...
&&(and) 연산자는 비교값이 모두 true여야 true를 반환하고,  
...
propNameOrNumber는 속성이나 배열 index를 나타내는 문자열, 숫자, 심볼 표현식이며,  
