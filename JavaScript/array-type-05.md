...
Array 인스턴스의 `concat()` 메서드는 `두 개 이상의 배열을 병합하는 데 사용`된다.  
...
`const newArr = arr.concat(["c", "d"]);`  
concat() 메서드는 arr 배열(['a'])에 전달받은 3개(['a', 'b', 'c'])의 파라미터를 병합해 새로운 배열을 반환.  
`["a"] + ["b", "c"] = ['a', 'b', 'c']`

concat() 메서드는 인자로 전달받은 값 순으로 배열 끝에서부터 값을 추가한다.

인자가 배열인 경우, 배열 안에 요소들을 꺼내어 새로운 배열에 포함시킨다.  
...
spread operator `...`은 배열의 요소를 각각 분해하여 개별 요소로 반환한다.  
